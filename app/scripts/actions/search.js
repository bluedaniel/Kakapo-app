...
const actions = {
  ...
  fetchService: (service, term) => dispatch => {
    return new Promise((resolve, reject) => {
      ...
      if (service === 'youtube') opts = { provider: getYoutubeSearch, type: constants.SEARCH_YOUTUBE };
      if (service === 'soundcloud') opts = { provider: getSoundCloudSearch, type: constants.SEARCH_SOUNDCLOUD };
      opts
        .provider(term)
  ...
  fetchServiceComplete: (items, type) => ({ type: type, items }),
